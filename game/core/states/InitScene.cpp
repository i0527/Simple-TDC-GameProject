...
void InitScene::Update(float deltaTime) {
    ...
    if (initState_.scanningCompleted && initState_.initializationStarted) {
        ...
        // スムーズな進捗バーアニメーション
        float targetProgress = (initState_.totalProgress > 0)
                                   ? (static_cast<float>(initState_.currentProgress) /
                                      initState_.totalProgress)
                                   : 0.0f;
        const float smoothSpeed = 5.0f; // アニメーション速度
        smoothProgress_ += (targetProgress - smoothProgress_) * smoothSpeed * deltaTime;
...
void InitScene::renderInitScreen() {
    ...
    systemAPI_->Render().DrawTextDefault(
        ...
        ToCoreColor({0, 0, 0, 100})); // 半透明黒のシャドウ
    systemAPI_->Render().DrawTextDefault(
        percentText, percentX, percentY, percentFontSize,
    ...
    systemAPI_->Render().DrawRectangle(
        barX + shadowOffset, barY + shadowOffset,
        barWidth, barHeight,
    ...
    if (progress > 0.0f) {
        ...
        systemAPI_->Render().DrawRectangle(
            barX, barY, fillWidth, barHeight * 0.3f, highlightColor);
    }
    ...
    systemAPI_->Render().DrawTextDefault(
        progressText, textX, textY, textFontSize,
    ...
    if (!pathText.empty()) {
        ...
        systemAPI_->Render().DrawTextDefault(
            pathText, pathX, pathY, pathFontSize,
    ...
    if (!initState_.initializationCompleted) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            ColorRGBA dotColor = ToCoreColor({
                static_cast<uint8_t>(ACCENT_BLUE.r),
            ...
            systemAPI_->Render().DrawCircle(
                indicatorX + offset, indicatorY, dotRadius, dotColor);
        }
    }
    ...
    systemAPI_->Render().DrawRectangle(
        panelX + panelShadowOffset, panelY + panelShadowOffset,
        panelWidth, panelHeight,
        ToCoreColor({0, 0, 0, 100})); // シャドウ
    ...
    systemAPI_->Render().DrawRectangleLines(
        panelX, panelY, panelWidth, panelHeight, 2.0f,
    ...
    Vec2 headerSize =
        systemAPI_->Render().MeasureTextDefaultCore(panelTitle, headerFontSize, 1.0f);
    float headerX = panelX + 30.0f;
    float headerY = panelY + 20.0f;
    systemAPI_->Render().DrawTextDefault(
        panelTitle, headerX, headerY, headerFontSize,
        ToCoreColor(TEXT_PRIMARY));

    // ヘッダー下の区切り線
    systemAPI_->Render().DrawLine(
        panelX + 20.0f, headerY + headerSize.y + 10.0f,
        panelX + panelWidth - 20.0f, headerY + headerSize.y + 10.0f,
        1.0f, ToCoreColor(DIVIDER));
    ...
    auto drawRow = [&](const char* label, const InitState::CategoryStats& stats,
        ...
        systemAPI_->Render().DrawTextDefault(
            nameText, nameX, rowY, rowFontSize,
            ToCoreColor(TEXT_MUTED));
...
void InitScene::renderErrorScreen() {
    ...
    // パネルボーダー（エラー色）
    systemAPI_->Render().DrawRectangleLines(
        panelX, panelY, panelWidth, panelHeight, 3.0f,
    ...
    systemAPI_->Render().DrawLine(
        ...
        lineThickness, ToCoreColor(DANGER_RED));
    systemAPI_->Render().DrawLine(
    ...
    // タイトルシャドウ
    systemAPI_->Render().DrawTextDefault(
        errorTitle, titleX + 2.0f, titleY + 2.0f, titleFontSize,
        ToCoreColor({0, 0, 0, 100}));
    systemAPI_->Render().DrawTextDefault(
        errorTitle, titleX, titleY, titleFontSize,
        ToCoreColor(DANGER_RED));
    ...
    systemAPI_->Render().DrawTextDefault(
        message, messageX, messageY, messageFontSize,
        ToCoreColor(TEXT_SECONDARY));

    // ========== 区切り線 ==========
    float dividerY = messageY + messageSize.y + 30.0f;
    systemAPI_->Render().DrawLine(
        panelX + 40.0f, dividerY,
        panelX + panelWidth - 40.0f, dividerY,
        1.0f, ToCoreColor(DIVIDER));
    ...
    systemAPI_->Render().DrawTextDefault(
        closeMessage, closeX, closeY, closeFontSize,
        ToCoreColor(TEXT_MUTED));
    ...
    ColorRGBA timeColor = ToCoreColor({
        static_cast<uint8_t>(WARNING_ORANGE.r),
    ...
    // 時間表示のシャドウ
    systemAPI_->Render().DrawTextDefault(
        timeText, timeX + 2.0f, timeY + 2.0f, timeFontSize,
        ToCoreColor({0, 0, 0, 100}));
    systemAPI_->Render().DrawTextDefault(
        timeText, timeX, timeY, timeFontSize, timeColor);
}
