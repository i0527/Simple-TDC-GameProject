...
TitleScreen::TitleScreen()
    : systemAPI_(nullptr), inputAPI_(nullptr), sharedContext_(nullptr),
      sceneOverlayAPI_(nullptr), isInitialized_(false),
      hasTransitionRequest_(false), requestedNextState_(GameState::Title),
      requestQuit_(false), background_texture_(nullptr),
...
bool TitleScreen::Initialize(BaseSystemAPI *systemAPI) {
  ...
  const float menuSpacing =
      320.0f; // メニュー頁E��間�E間隔�E��Eタン中忁E��の距離�E�E

  // ライセンスボタン�E�中央から左�E�E
  menu_items_[LICENSE].width = menuButtonWidth;
  menu_items_[LICENSE].height = menuButtonHeight;
  menu_items_[LICENSE].x = (centerX - menuSpacing) -
                           menuButtonWidth / 2.0f; // ボタン中忁E��ら左端を計箁E
  ...
  menu_items_[QUIT].x = (centerX + menuSpacing) -
                        menuButtonWidth / 2.0f; // ボタン中忁E��ら左端を計箁E
...
void TitleScreen::RenderBackground() {
  if (has_background_ && background_texture_ && background_texture_->id != 0) {
    // 画像背景描画
    Rect source = {0.0f, 0.0f, static_cast<float>(background_texture_->width),
                   static_cast<float>(background_texture_->height)};
    Rect dest = {0.0f, 0.0f, 1920.0f, 1080.0f};
    Vec2 origin = {0.0f, 0.0f};
    systemAPI_->Render().DrawTexturePro(*background_texture_, source, dest,
                                        origin, 0.0f, ToCoreColor(WHITE));
  } else {
    // グラチE�Eション背景
...
void TitleScreen::DrawGradientBackground() {
  ...
  ColorRGBA top_color =
      ToCoreColor(ui::OverlayColors::PANEL_BG_GITHUB); // 控えめ紺黁E
  ColorRGBA bottom_color = ToCoreColor(ui::OverlayColors::MAIN_BG); // 完璧な黁E

  systemAPI_->Render().DrawRectangleGradientV(0, 0, 1920, 1080, top_color,
                                              bottom_color);
}
...
void TitleScreen::RenderTitle() {
  ...
  systemAPI_->Render().DrawTextDefault(
      title_text_.c_str(), titleX, titleY, TITLE_FONT_SIZE,
...
void TitleScreen::RenderStartButton() {
  ...
  Rect dest = {start_button_.x, start_button_.y, start_button_.width,
               start_button_.height};
  systemAPI_->Render().DrawUiNineSlice(textureKey, dest, 8, 8, 8, 8,
                                       ToCoreColor(WHITE));
  ...
  float textY =
      start_button_.y + (start_button_.height - BUTTON_FONT_SIZE) / 2.0f;

  systemAPI_->Render().DrawTextDefault(
      start_button_.label.c_str(), textX, textY, BUTTON_FONT_SIZE,
...
void TitleScreen::RenderFooterMenu() {
  for (int i = 0; i < 3; ++i) {
    ...
    Rect dest = {menu_items_[i].x, menu_items_[i].y, menu_items_[i].width,
                 menu_items_[i].height};
    systemAPI_->Render().DrawUiNineSlice(textureKey, dest, 8, 8, 8, 8,
                                         ToCoreColor(WHITE));
    ...
    float textY =
        menu_items_[i].y + (menu_items_[i].height - MENU_FONT_SIZE) / 2.0f;

    systemAPI_->Render().DrawTextDefault(menu_items_[i].label.c_str(), textX,
                                         textY, MENU_FONT_SIZE, text_color);
  }
}
...
void TitleScreen::RenderVersionInfo() {
  ...
  systemAPI_->Render().DrawTextDefault(version_text_.c_str(), versionX,
                                       versionY, VERSION_FONT_SIZE,
