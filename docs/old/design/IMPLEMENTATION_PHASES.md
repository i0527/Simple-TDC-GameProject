...
### 解像度/ウィンドウポリシー（Phase1で確定）

- 許容解像度リストを 16:9 固定で運用：`1920x1080 / 1600x900 / 1280x720`
- 起動時は「モニタに収まる中で最大の候補」を選択し、見つからない場合は最小の `1280x720` にフォールバック
- ウィンドウリサイズは禁止（`FLAG_WINDOW_RESIZABLE` を付けない運用）。必要なら `SetWindowMinSize(1280, 720)` を併用可
- 解像度変更は設定メニュー実装後にのみ許可する方針。変更時は許容リストから選択して `SetWindowSize(newW, newH)` 後に `input.UpdateScreenSize(newW, newH)` を呼ぶ
- 仮想解像度は 1920x1080 を継続し、`RenderScaled` / `ScreenToVirtual` のレターボックス処理を維持
...
- [x] Worldクラス実装 または GameContextに統合
- [x] エンティティ名管理
- [x] コンポーネント操作ヘルパー
...
- [x] GameContextが正常に初期化・終了できる
- [x] 仮想FHD画面に図形を描画し、実画面にスケーリング表示できる
- [x] 2つ以上のシステムが登録・実行できる
- [x] メインループが60FPSで安定動作
...
- [x] DefinitionRegistryクラス実装
- [x] EntityDef, WaveDef, AbilityDef構造体定義
- [x] 登録・取得API
...
- [x] EntityLoader実装
- [x] WaveLoader実装
- [x] エラーハンドリング（try-catch必須＋フォールバック）
- [x] フォールバック生成方針：読込/パース失敗時に自動生成。論理的欠損（登録0件や全スキップ）時もfallback_entity/fallback_waveを生成。`DataLoaderBase::SetFallbackEnabled(false)` で抑止可、理由はTraceログに出力。
- [x] AbilityLoader拡張：critMultiplier/duration/isDot/isHot/modifiersなど拡張フィールドの読み込みとフォールバック対応
...
- [x] 基本的なJSON検証（必須フィールド、型チェック、配列空チェック）
- [x] 具体的なエラーメッセージ生成と重複ID警告
- [x] 数値範囲チェック（Entity health>0、Wave delay>=0、Ability cost/cooldown/power/critMultiplier/duration>=0、modifiers型検証）
...
- [x] ファイル/ディレクトリ監視（ポーリング）
- [x] 変更検出とコールバック（個別＋バッチ）
- [x] ポーリング間隔設定、missing警告抑止、初回/再検出ログ強化
