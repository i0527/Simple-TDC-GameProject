...
### 役割分離メモ（状態タイムラインとの関係）
- モーション内イベント: モーションに内包されるエフェクト/SE/ヒットボックス/カスタム。クリップ固有の挙動に限定。
- 状態タイムライン（エンティティ側）: 状態開始からの時刻で発火する追加イベント（effect/sound/hitbox/custom）。モーション外の演出や外部連携をここで扱う。
- 再生時: 状態→モーション/スプライトを選択し、モーションイベントと状態タイムラインイベントを並行で処理する。二重定義は設計側でルール化（例: ヒットボックスはモーション側を優先し、状態タイムラインでは重ねない）。
