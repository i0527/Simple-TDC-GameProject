...
> LineTD はレーン1本固定。レーンのY位置はレイアウトの固定値から決まるため、ステージJSONにY座標を持たせない。
...
public:
    void Update(Core::GameContext& context, float deltaTime) override {
        ...
        for (auto entity : view) {
            ...
            // レーン情報取得（1レーン固定だが開始/終端Xはここから参照）
            auto* lane = world.GetComponent<Lane>(entity);
            if (!lane) continue;
            const float laneY = context.GetLayout().GetLineTDLaneY();  // JSONでは与えず固定値
...
- レーン: 1本固定（`laneId=0`）。Y位置はレイアウトの固定値から取得し、JSONでは扱わない。
  ...
  "waves": [
    {
      ...
      "startTime": 0.0,
      "spawns": [
        { "time": 0.0, "enemyId": "slime_s", "count": 3, "interval": 1.0 },
        { "time": 5.0, "enemyId": "slime_m", "count": 1 },
        { "time": 8.0, "enemyId": "bat_s", "count": 2, "interval": 0.5 }
      ],
      "boss": false
    },
    {
      ...
      "startTime": 25.0,
      "spawns": [
        { "time": 0.0, "enemyId": "slime_boss", "count": 1 }
      ],
      "boss": true
    }
  ],
...
- LineTD は 1 レーン固定。`lane` キーは省略推奨（記載されていても0扱い）、`y` はレイアウト固定値から決定するため JSON で指定しない。
...
- `LaneManagerSystem`: 単一レーンの開始/終端Xを固定レイアウトから供給（JSON入力不要）
...
- [ ] `waves`/`cost` JSON ロードとスキーマ検証（レーン定義は不要）
