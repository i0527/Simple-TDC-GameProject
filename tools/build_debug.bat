@echo off
REM ============================================================================
REM Simple-TDC-GameProject - Debug Build Script (VS2026 Build Tools)
REM ============================================================================
REM 使用方法: build_debug.bat [clean]
...
if errorlevel 1 (
    ...
    echo CMake をインストールするか、PATH に追加してください
    exit /b 1
)
...
where ninja >nul 2>&1
if errorlevel 1 (
    echo [WARNING] Ninja が見つかりません。Visual Studio ジェネレータを使用します
    set "CMAKE_GENERATOR=Visual Studio 19 2026"
    set "CMAKE_ARGS=-A x64"
) else (
    echo [INFO] Ninja を使用します
...
cmake -G "%CMAKE_GENERATOR%" %CMAKE_ARGS% -DCMAKE_BUILD_TYPE=Debug -B "%BUILD_DIR%" -S "%PROJECT_ROOT%"
