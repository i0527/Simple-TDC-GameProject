 ...
 * @brief 破棄システム
 */
void DestructionSystem(Core::World &world, Core::GameContext &context,
                       float dt) {
...
UnifiedGame::UnifiedGame()
    : world_(std::make_unique<Core::World>()),
      ...
      definitionLoader_(
          std::make_unique<Data::DefinitionLoader>(*definitions_)) {
  // SystemRunnerはRunPhase/RunAll呼び出し時にworld_とcontext_を引数で受け取る
}

bool UnifiedGame::Initialize(const std::string &definitionsPath) {
  if (initialized_) {
  ...
  if (homeScene) {
    homeScene->SetGameModeCallback(
        [this](GameMode mode) { this->SetGameMode(mode); });
  }

  // 開発者モード初期化
  devMode_ = std::make_unique<Game::DevMode::DevModeManager>();
  devMode_->Initialize(*context_, definitions_.get(), definitionLoader_.get());
...
void UnifiedGame::RegisterTDSystems() {
  ...
  auto &gameStateManager =
      context_->Emplace<Domain::TD::Managers::GameStateManager>();

  // TDシステムラッパー
  systemRunner_->Register(
      Core::SystemPhase::Update, "WaveManager",
  ...
  systemRunner_->Register(
      Core::SystemPhase::Update, "SpawnManager",
  ...
  systemRunner_->Register(
      Core::SystemPhase::Update, "GameStateManager",
      [&gameStateManager, &waveManager](Core::World &world,
...
void UnifiedGame::RegisterRoguelikeSystems() {
  // Roguelikeマネージャーを登録
  auto &turnManager =
      context_->Emplace<Domain::Roguelike::Managers::TurnManager>();
...
void UnifiedGame::RegisterScene(const std::string &name,
...
void UnifiedGame::ChangeScene(const std::string &name) {
  ...
  if (!currentSceneName_.empty() &&
      scenes_.find(currentSceneName_) != scenes_.end()) {
...
void UnifiedGame::Update(float deltaTime) {
  ...
  // システム実行（Render以外）
  systemRunner_->RunPhase(Core::SystemPhase::PreUpdate, *world_, *context_,
                          deltaTime);
  systemRunner_->RunPhase(Core::SystemPhase::Update, *world_, *context_,
                          deltaTime);
  systemRunner_->RunPhase(Core::SystemPhase::PostUpdate, *world_, *context_,
                          deltaTime);
}
...
void UnifiedGame::Render() {
  ...
  // FHDレンダリング終了（EndTextureMode + BeginDrawing + DrawTexturePro +
  // EndDrawingが呼ばれる）
  renderer->EndRender();
...
nlohmann::json UnifiedGame::GetGameState() const {
  ...
  for (auto entity : rogueView) {
    ...
    if (world_->HasAll<Domain::Roguelike::Components::TurnActor>(entity)) {
      const auto *actor =
          world_->TryGet<Domain::Roguelike::Components::TurnActor>(entity);
  ...
  if (currentMode_ == GameMode::TD) {
    ...
    if (context_->Has<Domain::TD::Managers::WaveManager>()) {
      const auto &waveManager =
          context_->Get<Domain::TD::Managers::WaveManager>();
      tdState["waveManager"] = nlohmann::json::object();
      tdState["waveManager"]["currentWave"] =
    ...
    if (context_->Has<Domain::TD::Managers::GameStateManager>()) {
      const auto &gameStateManager =
          context_->Get<Domain::TD::Managers::GameStateManager>();
      tdState["gameStateManager"] = nlohmann::json::object();
      tdState["gameStateManager"]["baseHealth"] =
          gameStateManager.GetBaseHealth();
      tdState["gameStateManager"]["enemyBaseHealth"] =
          gameStateManager.GetEnemyBaseHealth();
      tdState["gameStateManager"]["phase"] =
          static_cast<int>(gameStateManager.GetPhase());
    }

    // SpawnManagerの情報を取得
    if (context_->Has<Domain::TD::Managers::SpawnManager>()) {
      const auto &spawnManager =
          context_->Get<Domain::TD::Managers::SpawnManager>();
  ...
  if (currentMode_ == GameMode::Roguelike) {
    ...
    if (context_->Has<Domain::Roguelike::Managers::TurnManager>()) {
      const auto &turnManager =
          context_->Get<Domain::Roguelike::Managers::TurnManager>();
      roguelikeState["turnManager"] = nlohmann::json::object();
      roguelikeState["turnManager"]["turnCount"] = turnManager.GetTurnCount();
      roguelikeState["turnManager"]["state"] =
          static_cast<int>(turnManager.GetState());
      roguelikeState["turnManager"]["awaitingInput"] =
          turnManager.IsAwaitingInput();
    }
...
nlohmann::json UnifiedGame::GetEntities() const {
  ...
  for (auto entity : view) {
    ...
    // Roguelikeコンポーネント
    if (auto *rogueStats =
            world_->TryGet<Domain::Roguelike::Components::Stats>(entity)) {
...
nlohmann::json UnifiedGame::GetEntity(const std::string &entityId) const {
  ...
  try {
    ...
    if (auto *gridPos =
            world_->TryGet<Domain::Roguelike::Components::GridPosition>(
                entity)) {
...
nlohmann::json UnifiedGame::CreateEntity(const nlohmann::json &config) const {
  ...
  // 基本コンポーネント
  std::string entityId = config.value(
      "entityId",
      "test_entity_" + std::to_string(static_cast<uint32_t>(entity)));
  ...
  if (config.contains("roguelike")) {
    ...
    if (rogueConfig.contains("gridPosition")) {
      ...
      world_->Emplace<Domain::Roguelike::Components::GridPosition>(
          entity, gridX, gridY);
    }
  }
...
nlohmann::json UnifiedGame::UpdateEntity(const std::string &entityId,
  ...
  try {
    ...
    if (config.contains("td") && config["td"].contains("stats")) {
      ...
      if (stats) {
        ...
        if (statsConfig.contains("maxHealth"))
          stats->maxHealth = statsConfig["maxHealth"];
        if (statsConfig.contains("currentHealth"))
          stats->currentHealth = statsConfig["currentHealth"];
        if (statsConfig.contains("attack"))
          stats->attack = statsConfig["attack"];
        if (statsConfig.contains("defense"))
          stats->defense = statsConfig["defense"];
        if (statsConfig.contains("moveSpeed"))
          stats->moveSpeed = statsConfig["moveSpeed"];
        if (statsConfig.contains("attackInterval"))
          stats->attackInterval = statsConfig["attackInterval"];
      }
    }
    ...
    if (config.contains("roguelike") && config["roguelike"].contains("stats")) {
      ...
      if (stats) {
        ...
        if (statsConfig.contains("maxHp"))
          stats->maxHp = statsConfig["maxHp"];
        if (statsConfig.contains("hp"))
          stats->hp = statsConfig["hp"];
        if (statsConfig.contains("attack"))
          stats->attack = statsConfig["attack"];
        if (statsConfig.contains("defense"))
          stats->defense = statsConfig["defense"];
        if (statsConfig.contains("level"))
          stats->level = statsConfig["level"];
        if (statsConfig.contains("experience"))
          stats->experience = statsConfig["experience"];
      }
    }
...
nlohmann::json
UnifiedGame::SpawnEntity(const std::string &entityId,
                         const nlohmann::json &spawnParams) const {
  ...
  if (context_->Has<Core::EntityFactory>()) {
    ...
    entt::entity entity =
        factory.CreateCharacter(definitionId, x, y, isEnemy, level);
...
nlohmann::json UnifiedGame::SetEntityStats(const std::string &entityId,
                                           const nlohmann::json &stats) const {
  try {
    uint32_t entityValue = std::stoul(entityId);
    entt::entity entity = static_cast<entt::entity>(entityValue);
    ...
    if (auto *tdStats = world_->TryGet<Domain::TD::Components::Stats>(entity)) {
      if (stats.contains("maxHealth"))
        tdStats->maxHealth = stats["maxHealth"];
      if (stats.contains("currentHealth"))
        tdStats->currentHealth = stats["currentHealth"];
      if (stats.contains("attack"))
        tdStats->attack = stats["attack"];
      if (stats.contains("defense"))
        tdStats->defense = stats["defense"];
      if (stats.contains("moveSpeed"))
        tdStats->moveSpeed = stats["moveSpeed"];
      if (stats.contains("attackInterval"))
        tdStats->attackInterval = stats["attackInterval"];
    }

    // Roguelikeステータス設定
    if (auto *rogueStats =
            world_->TryGet<Domain::Roguelike::Components::Stats>(entity)) {
      if (stats.contains("maxHp"))
        rogueStats->maxHp = stats["maxHp"];
      if (stats.contains("hp"))
        rogueStats->hp = stats["hp"];
      if (stats.contains("attack"))
        rogueStats->attack = stats["attack"];
      if (stats.contains("defense"))
        rogueStats->defense = stats["defense"];
      if (stats.contains("level"))
        rogueStats->level = stats["level"];
      if (stats.contains("experience"))
        rogueStats->experience = stats["experience"];
    }
...
nlohmann::json UnifiedGame::SetEntityAI(const std::string &entityId,
                                        const nlohmann::json &aiConfig) const {
...
nlohmann::json
UnifiedGame::SetEntitySkills(const std::string &entityId,
                             const nlohmann::json &skills) const {
