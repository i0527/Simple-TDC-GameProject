...
- 役割を「レイアウト配置・スタイル（色/フォント/余白）・バインドキー宣言・イベント名宣言」に限定する。
- バインドはキー参照のみ（式なし）。複雑な式評価は行わず、値整形はネイティブ側で実施。
- UIイベントは最小セット（ButtonClicked/ToggleChanged など）を宣言し、実装はネイティブ側に委譲する。
...
### 1.4 シーン/状態連動ルール（常駐運用）

- 常駐前提: F1〜F4 エディタは常駐し、現在のシーンと戦闘状態（Ready/Playing/Paused/Victory/Defeat）を参照してプレビュー対象を決定する。
- シーン切替時: プレビュー対象リソース（UIレイアウト/ステージ/エンティティ/モーション）と入力バインドを再設定し、SceneManager へフックして自動更新する。シーン側はエディタを停止せず、プレビュー/バインドのみ差し替える。
- 状態変化時: Paused で編集時はゲーム側を安全停止、Playing 復帰時にプレビューと入力バインドを元に戻す。Victory/Defeat 時は Result 表示用のプレビューへ切替。
- HotReload 順序: エディタ保存 → ローカル検証 → ゲーム側再検証 → 成功時のみ適用。失敗時はゲーム側データをロールバックし、エディタへエラーを返す。OnHotReload は現在シーンへ伝播し、必要なら安全側（Paused 等）に倒す。
- 入力/カメラ: エディタとゲームの入力マップが衝突しないよう、シーン切替時に優先度とバインドを再適用。プレビューカメラもシーンに合わせて初期化する。
- コンテキスト通知: シーン切替時に `sceneId`, `gameState`, `stageId`, `tdMode` をエディタへ渡し、プレビュー対象を即座に更新する。戦闘中のホットリロードはステージ定義のみ許可し、編成変更は次戦闘から適用する。
...
- 状態マッピング: 状態ごとに motionClipId または spriteAnimId を割当
- 状態タイムライン: t 秒後に発火するイベント（effect/sound/hitbox/custom）の追加・編集
- 図鑑/編成メタ: displayName, shortDesc, icons[], portraits[], voiceId, tags[] を編集しプレビュー表示
